<?php
  ...
  if ($chartdata == "1") {
    ...
    $sqlCountScoreInQuestionSet2 = "SELECT count(score_value) as countvalue, s.score_value FROM score s, question q, questionSet qs WHERE qs.question_set_id = '$questionsetid'
    ...
    $queryCountScoreInQuestionSet2 = mysql_query($sqlCountScoreInQuestionSet2);
    $countCountScoreInQuestionSet2 = mysql_num_rows($queryCountScoreInQuestionSet2);

    if ($countCountScoreInQuestionSet2 == 0) {
    ...
    while ($resultGetQuestionInQuestionSet = mysql_fetch_array($queryGetQuestionInQuestionSet)) {
      ...
      for ($second = $start; $second <= $end; $second += 86400) {
        $endsecond = $second + 86400;
        $sdate = date("Y-m-d", $second);
        $edate = date("Y-m-d", $endsecond);
        // $sqlCountScore = "SELECT count(score_value) as countvalue, score_value FROM score s WHERE score_question_id = '$questionid' AND score_datetime >= '$startdatetime'
        //                   AND score_datetime <= '$enddatetime' GROUP BY score_value ORDER BY score_value";

        $sqlCountScore = "SELECT count(score_value) as countvalue, score_value, DATE(s.score_datetime) as day FROM score s WHERE score_question_id = '$questionid' AND score_datetime >= '$sdate'
                          AND score_datetime <= '$edate' GROUP BY score_value, DATE(s.score_datetime) ORDER BY DATE(s.score_datetime) ASC, s.score_value";
        $queryCountScore = mysql_query($sqlCountScore);
        $counrRowCountScore = mysql_num_rows($queryCountScore);

        $scoreColumnCount = $columnCount;

        if ($counrRowCountScore != 0) {
          $rowCount2 = $rowCount;
          $objPHPExcel->getActiveSheet()->setCellValue($scoreColumnCount . $rowCount2, 'วันที่');
          $rowCount2 = $rowCount2 + 1;
          $objPHPExcel->getActiveSheet()->setCellValue($scoreColumnCount . $rowCount2, $sdate);
          $scoreColumnCount++;
        }
    ...
    for ($second2 = $start; $second2 <= $end; $second2 += 86400) {
      $endsecond2 = $second2 + 86400;
      $sdate2 = date("Y-m-d", $second2);
      $edate2 = date("Y-m-d", $endsecond2);
      ...
      $sqlCountScoreInQuestionSet = "SELECT count(score_value) as countvalue, s.score_value FROM score s, question q, questionSet qs WHERE qs.question_set_id = '$questionsetid'
                                    AND s.score_datetime >= '$sdate2' AND s.score_datetime <= '$edate2' AND s.score_question_id = q.question_id
                                    AND q.question_question_set_id = qs.question_set_id GROUP BY s.score_value";
      ...
      if ($countCountScoreInQuestionSet != 0) {
        $rowCount3 = $rowCount;
        $objPHPExcel->getActiveSheet()->setCellValue($sumColumnCount . $rowCount3, 'วันที่');
        $rowCount3 = $rowCount3 + 1;
        $objPHPExcel->getActiveSheet()->setCellValue($sumColumnCount . $rowCount3, $sdate2);
        $sumColumnCount++;
      }
    ...
    if ($commentFlag == "1") {
      ...
      $objWorksheet->setCellValue('A2', 'วันที่');
      $objWorksheet->setCellValue('B2', 'ลำดับ');
      $objWorksheet->setCellValue('C2', 'ข้อเสนอแนะเพิ่มเติม');
      $rowCount = $rowCount + 1;
      $index = 1;

      for ($second3 = $start; $second3 <= $end; $second3 += 86400) {
        $endsecond3 = $second3 + 86400;
        $sdate3 = date("Y-m-d", $second3);
        $edate3 = date("Y-m-d", $endsecond3);

        $sqlGetComment = "SELECT * FROM comment WHERE comment_question_set_id = '$questionsetid' AND comment_datetime >= '$sdate3' AND comment_datetime <= '$edate3' ORDER BY comment_datetime";
        $queryGetComment = mysql_query($sqlGetComment);
        ...
        if ($countRowGetComment != 0) {
          $objWorksheet->setCellValue('A' . $rowCount, $sdate3);
          // $rowCount++;
        }
